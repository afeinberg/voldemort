...
if [ -z "$VOLD_OPTS" ]; then
  VOLD_OPTS="-Xmx18G -server -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=5050 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -XX:NewSize=2048m -XX:MaxNewSize=2048m -XX:+UseConcMarkSweepGC -XX:+UseParNewGC -XX:CMSInitiatingOccupancyFraction=70 -XX:SurvivorRatio=2"
fi
